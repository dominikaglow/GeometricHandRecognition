...
def compare_features(features1, features2):
    """
    Compare the LBP and Gabor features of two images and return their combined distance.
    """
    chi_lbp = chi_square_distance(features1["lbp"], features2["lbp"])
    chi_gabor = sum(chi_square_distance(g1, g2) for g1, g2 in zip(features1["gabor"], features2["gabor"]))
    return 0.5 * chi_lbp + 0.5 * chi_gabor
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python find_similar_pairs.py <features_json>")
        sys.exit(1)

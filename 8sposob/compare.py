...
def compare_features(features1, features2):
    """
    Compare the LBP, HOG, and Color Histogram features of two images and return their combined distance.
    """
    chi_lbp = chi_square_distance(features1["lbp"], features2["lbp"])
    chi_hog = chi_square_distance(features1["hog"], features2["hog"])
    chi_color_histogram = chi_square_distance(features1["color_histogram"], features2["color_histogram"])
    # Giving more weight to LBP features
    return (0.92 * chi_lbp + 0.00 * chi_hog + 0.08 * chi_color_histogram)
...
def find_most_similar_images(features_data):
    ...
    for i in range(total_images):
        ...
        print("Correctly classified: {}/{} ({:.2%})".format(correct_matches, i + 1, correct_matches / (i + 1)))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python compare_combined.py <features_json>")
        sys.exit(1)

...
def compare_features(features1, features2):
    """
    Compare the hand-geometry and palmprint features of two images and return their combined distance.
    """
    hand_geometry_distance = euclidean_distance(features1["hand_geometry"], features2["hand_geometry"])
    palmprint_distance = chi_square_distance(features1["palmprint"], features2["palmprint"])
    return 0.00 * hand_geometry_distance + 1 * palmprint_distance
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python compare_hand_palm_features.py <features_json>")
        sys.exit(1)
